...
> [!IMPORTANT]
> This repository is maintained as the `rtgiskard/authelia` development fork. It preserves the Authelia binary,
> configuration, and Go module names for deployment compatibility, but the fork product direction is File backend-first
> admin user management with no LDAP management surface. See [FORK.md](FORK.md) for the current fork scope, roadmap, and
