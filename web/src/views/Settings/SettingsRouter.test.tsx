...
vi.mock("@layouts/SettingsLayout", () => ({
    default: (props: { administrationEnabled?: boolean; children?: ReactNode }) => (
        <div data-administration-enabled={String(props.administrationEnabled)} data-testid="settings-layout">
            {props.children}
        </div>
    ),
}));
...
beforeEach(() => {
    ...
    vi.mocked(useConfiguration).mockReturnValue([undefined, mockFetchConfiguration, false, undefined]);
});
...
it("passes administration availability to the settings layout", async () => {
    ...
    expect(screen.getByTestId("settings-layout")).toHaveAttribute("data-administration-enabled", "true");
