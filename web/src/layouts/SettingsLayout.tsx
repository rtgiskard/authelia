...
const SettingsLayout = (props: Props) => {
	...
	useEffect(() => {
		document.title = translate("Settings - {{authelia}}", {
			authelia: atob(String.fromCodePoint(...EncodedName)),
		});
	}, [translate]);

	const drawerWidth = props.drawerWidth ?? defaultDrawerWidth;
	const drawerNavItems = props.administrationEnabled
		? navItems
		: navItems.filter((item) => item.keyname !== "user-management");
...
const DrawerNavItem = (props: NavItem) => {
	const selected =
		globalThis.location.pathname === props.pathname ||
		globalThis.location.pathname === `${props.pathname}/`;
